...
import { ensureSchema, pruneJoinEvents } from "./db.ts";
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === "POST" && url.pathname === config.webhookPath) {
      ...
      try {
        // Схема не создаётся при импорте (сеть в global scope запрещена) — гарантируем
        // её лениво, один раз на изолят, до первого обращения к БД в хендлерах.
        await ensureSchema();
  ...
  // Cron Trigger (ежедневно): чистим журнал заявок старше ретеншена и заодно
  // переустанавливаем вебхук/команды — единственный гарантированный периодический
  // вызов, поэтому он же само-восстанавливает обвязку Telegram, если она слетела
  // (после /revoke токена или забытого `bun run setup`). waitUntil держит воркер
  // живым до конца работы, не блокируя возврат из хендлера.
  async scheduled(_event: unknown, _env: unknown, ctx: ExecutionContext): Promise<void> {
    ctx.waitUntil(runDailyMaintenance());
  },
};
...
async function runDailyMaintenance(): Promise<void> {
  try {
    ...
    console.error("Ошибка чистки журнала:", err);
  }

  // Независимо от чистки — переустанавливаем вебхук и команды.
  try {
    await configureTelegram(bot);
  } catch (err) {
    console.error("Не удалось переустановить вебхук/команды:", err);
  }
}
